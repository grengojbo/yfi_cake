...
class PDF_Generic extends FPDF {
    ...
    function addAccessProvider( $ap_detail)
    {
        ...
        $address = $ap_detail['street_no'].' '.$ap_detail['street']."\n".$ap_detail['town_suburb']."\n".$ap_detail['city'];
        $this->MultiCell($max_address_width,3,iconv('UTF-8', $font_encode,$address),$outline,2);
    ...
    function addVoucher($username,$voucher)
    {
        ...
        if(array_key_exists('days_valid',$voucher)){
            $this->SetFont( $font_type, $font_format_i, $text_size);
            $this->SetX($x_p);
