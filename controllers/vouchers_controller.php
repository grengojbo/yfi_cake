    ...
    */
     ...
     function json_index(){
       ...
       $json_return   = array();
        ...
        foreach($list as $item){
            ...
            array_push($json_return['items'],array('id'=> $id,'profile'=> $profile, 'creator'=>$creator,'username' => $username,'realm' => $realm_name,'status' => $status,'created'=>$created));
    ...
    function _returnSearchFilterConditions(){
        ...
        if(array_key_exists('realm',$this->params['url'])){
         ...
         if(array_key_exists('created',$this->params['url'])){

            $column     = 'Voucher.created';
            $condition  = $this->params['url']['created'];
