...
class CmpPermanentComponent extends Object {
    ...
    function add_permanent($permanent_info){
        ...
        //Add the expire_on date as a check value //We will already send it as a UNUX timestamp
        $exp['Radcheck']['username']    = $full_user;
        $exp['Radcheck']['attribute']   = 'Expiration';
        $exp['Radcheck']['op']          = ':=';
        $exp['Radcheck']['value']       = $permanent_info['expire_on'];
    ...
    function delete_user_by_username($username,$accounting_also=true){
        $u  = $this->controller->User->find('count',array('conditions'=> array('User.username' => $username)));
        if($u > 0){
            ...
            $this->controller->Radcheck->deleteAll(     array("username"=>"$username"),false);
            $this->controller->Radreply->deleteAll(     array("username"=>"$username"),true);
            if($accounting_also == true){ //Delete by default the accounting records
                $this->controller->Radacct->deleteAll(      array("username"=>"$username"),true);
            }

            //Remove Devices (for MAC authentication)
            $this->controller->Device->deleteAll(       array("Device.user_id" => $id),true);
            //Remove all Intenet credits related to this user
            $this->controller->Credit->deleteAll(       array("Credit.used_by_id" => $id),true);
            $this->controller->User->del($id,true);
            $this->controller->Radusergroup->removeUser($username);
            return true;
        }else{
            return false;
        }
