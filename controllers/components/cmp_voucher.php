...
class CmpVoucherComponent extends Object {
    ...
    function add_voucher($voucher_info){
        ...
        */
        ...
        $dup_count = $this->controller->Radcheck->find('count',array('conditions' => array('attribute'=> 'Cleartext-Password', 'value' => $voucher_password)));
        ...
        if(array_key_exists('iso_format',$voucher_info)){
    ...
    function _get_usage($username,$yfi_voucher){
        ...
        $voucher_time_total;
        $first_login_time ='';
        $acct = $this->controller->Radacct->find('all',array(
                                                    'conditions' => array('Radacct.username'   => $username),
                                                    'order'         => array('Radacct.acctstarttime ASC')   //Add an order clause
                ));
	    ...
	    if ($time_cap == 0) {                       //This is a voucher that was specified and 0 days expiry after first login
		    $time_cap = "NA";                       //Vouchers will always have a Yfi-Voucher attribute even if it specifies 0-00-00-00
	    }else{
            $time_cap     = $time_cap - $time_used; //The time cap is returned if Yfi-Voucher had a value of 1-00-00-00 or bigger....
		    if ($time_cap < 0) {                    //Avoid negative values
			    $time_cap = 0;
		    }
	    }
        ...
        //------------------------------------------------------------------------------------
        $return_array = array(
            'expiryafter'	=> $time_cap,       //23/2/12 We split the reporting on the time left after first login (if not 0-00-00-00)
            'time_used'     => $time_used,      //And the time remaining if the voucher's profile has a time component as well
